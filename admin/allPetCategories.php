                ...
                <table id="allposts" class="table table-striped table-bordered" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <?php $check_id = $row['id']; ?>
                            <?php $query_check = "SELECT * FROM pets WHERE category = '$check_id'";
                            $select_check = $db->select($query_check); ?>
                            <th><a href="editPetCategory.php?id=<?php echo urlencode($row['id']);?>">Edit</a>
                            ||
                            <?php if ($select_check == NULL): ?>
                            <a onclick="return confirm('Are You Sure You Want To Delete?');" href="deletePetcategory.php?id=<?php echo urlencode($row['id']);?>">Delete</a>
                            <?php else: ?>
